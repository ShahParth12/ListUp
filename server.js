...
const { queueSave, loadAllRooms, deleteRoomFromDb, syncHostSocketId } = require("./lib/roomDb");
...
function normalizeAnswer(text) {
  ...
  return text.trim().toLowerCase().replace(/^(a |an |the )/i, "");
}
...
// Returns true if answer is valid (starts with the round letter after ignoring articles)
function isValidAnswer(answer, letter) {
  const first = getFirstLetter(answer);
  return first === letter.toLowerCase();
}

// Score a single category: count how many players had unique valid answers.
// Returns { byPlayerId: { [playerId]: { valid, normalized, points } } }
function scoreCategory(answers, letter) {
  const validAnswers = answers
    .filter((a) => a.answer && isValidAnswer(a.answer, letter))
    .map((a) => ({ ...a, normalized: normalizeAnswer(a.answer) }));
  const counts = {};
  validAnswers.forEach((a) => { counts[a.normalized] = (counts[a.normalized] || 0) + 1; });
  const byPlayerId = {};
  answers.forEach((a) => {
    const valid = !!(a.answer && isValidAnswer(a.answer, letter));
    const normalized = valid ? normalizeAnswer(a.answer) : "";
    const points = valid && counts[normalized] === 1 ? 1 : 0;
    byPlayerId[a.playerId] = { valid, normalized, points };
  });
  return byPlayerId;
}
...
const DISCONNECT_REMOVE_AFTER_MS = 90 * 1000; // After 90s absent, treat as left (remove and add to recentlyDisconnected)
...
io.on("connection", (socket) => {
  ...
  socket.on("rejoin_room", (roomCode, playerName, cb) => {
    ...
    if (disconnectedSlot) {
      if (disconnectedSlot.disconnectTimeoutId) {
        clearTimeout(disconnectedSlot.disconnectTimeoutId);
  ...
  socket.on("start_game", () => {
    ...
    const t = setTimeout(() => endRoundForRoom(room), ROUND_START_COUNTDOWN_MS + roundSeconds * 1000);
  ...
  socket.on("submit_answers", (answers) => {
    ...
    const incoming = answers.slice(0, room.categories.length);
    ...
    if (submittedCount >= activePlayers.length && activePlayers.length > 0) {
      ...
      setTimeout(() => endRoundForRoom(room), 1500);
    }
  });
  ...
  socket.on("next_round", (payload, cb) => {
    ...
    if (removeDisconnected && disconnectedActive.length > 0) {
      disconnectedActive.forEach((p) => {
        ...
        if (p.disconnectTimeoutId) {
          clearTimeout(p.disconnectTimeoutId);
  ...
  socket.on("disconnect", () => {
    ...
    // Mark as disconnected so they stay in the room; rejoin_room can reattach same name.
    player.disconnected = true;
    playerToRoom.delete(socket.id);
    const oldId = socket.id;
    const playerSeatId = player.seatId;
    const playerName = player.name;
    const playerScore = player.score ?? 0;
    player.disconnectTimeoutId = setTimeout(() => {
      player.disconnectTimeoutId = null;
      room.players = room.players.filter((p) => p.seatId !== playerSeatId);
      if (room.players.length === 0) {
        rooms.delete(code);
        recentlyDisconnected.delete(code);
        deleteRoomFromDb(code);
      ...
      list.push({ id: oldId, name: playerName, score: playerScore });
      if (list.length > MAX_RECENT_DISCONNECTS) list.shift();
      if (room.hostSeatId === playerSeatId) {
        room.hostSeatId = room.players[0].seatId;
      }
      syncHostSocketId(room);
      delete room.answers?.[playerSeatId];
      persistRoom(room);
      io.to(code).emit("player_toast", { type: "leave", name: playerName });
      if (room.phase === "correction") {
        const activePlayers = room.players.filter((p) => !p.waitingForNextRound && !p.disconnected);
        if (activePlayers.length < 2) {
          finishCorrectionAndGoToScoring(room);
      ...
      logRoom(room, "disconnect_timeout_remove", { by: `${playerName}(${oldId.slice(0, 6)})` });
    }, DISCONNECT_REMOVE_AFTER_MS);

    persistRoom(room);
    emitRoomStateToRoom(room);
    logRoom(room, "disconnect_marked", { by: playerLabel(player), removeAfterMs: DISCONNECT_REMOVE_AFTER_MS });
  });
});
...
function restoreRoundTimerIfNeeded(room) {
  ...
  if (remaining > 0) {
    const t = setTimeout(() => endRoundForRoom(room), remaining);
    roundTimeouts.set(code, t);
  } else {
    setTimeout(() => endRoundForRoom(room), 0);
  }
}

function restoreDisconnectTimers(room) {
  room.players.forEach((p) => {
    if (!p.disconnected) return;
    const playerSeatId = p.seatId;
    const playerName = p.name;
    const playerScore = p.score ?? 0;
    const code = room.roomCode;
    p.disconnectTimeoutId = setTimeout(() => {
      p.disconnectTimeoutId = null;
      room.players = room.players.filter((x) => x.seatId !== playerSeatId);
      if (room.players.length === 0) {
        rooms.delete(code);
        recentlyDisconnected.delete(code);
        deleteRoomFromDb(code);
      ...
      list.push({ id: p.id, name: playerName, score: playerScore });
      if (list.length > MAX_RECENT_DISCONNECTS) list.shift();
      if (room.hostSeatId === playerSeatId) {
        room.hostSeatId = room.players[0].seatId;
      }
      ...
      io.to(code).emit("player_toast", { type: "leave", name: playerName });
      if (room.phase === "correction") {
        const activePlayers = room.players.filter((pl) => !pl.waitingForNextRound && !pl.disconnected);
        if (activePlayers.length < 2) {
          finishCorrectionAndGoToScoring(room);
          return;
        }
