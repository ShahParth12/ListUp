...
function roomToRow(room, recentlyDisconnectedForRoom) {
  return {
    ...
    goldenBuzzers:
      room.goldenBuzzers &&
      typeof room.goldenBuzzers === "object" &&
      Object.keys(room.goldenBuzzers).length > 0
        ? jsonStr(room.goldenBuzzers)
        : null
  };
}
...
function rowToRoom(row) {
  ...
  return {
    ...
    goldenBuzzers: (() => {
      const g = jsonParse(row.goldenBuzzers, null);
...
/** Coalesce rapid persistRoom calls into one DB write per room per window. */
const SAVE_DEBOUNCE_MS = 400;
/** @type {Map<string, { timer: ReturnType<typeof setTimeout>, room: object, recent: unknown }>} */
const pendingSaves = new Map();

function queueSave(room, recentlyDisconnected) {
  const code = room.roomCode;
  if (!code) return;
  const recent = recentlyDisconnected.get(code) || null;
  let entry = pendingSaves.get(code);
  if (entry) {
    entry.room = room;
    entry.recent = recent;
    return;
  }
  entry = { room, recent, timer: null };
  entry.timer = setTimeout(() => {
    pendingSaves.delete(code);
    saveRoom(entry.room, entry.recent).catch(() => {});
  }, SAVE_DEBOUNCE_MS);
  pendingSaves.set(code, entry);
}

function cancelPendingSave(code) {
  const entry = pendingSaves.get(code);
  if (entry) {
...
async function flushRoomSave(code) {
  const entry = pendingSaves.get(code);
  if (!entry) return;
  clearTimeout(entry.timer);
  pendingSaves.delete(code);
  await saveRoom(entry.room, entry.recent);
}

async function flushAllPendingSaves() {
  const codes = [...pendingSaves.keys()];
  for (const c of codes) {
    await flushRoomSave(c);
  }
}
